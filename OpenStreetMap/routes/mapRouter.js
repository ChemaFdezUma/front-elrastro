...
router.get("/coordenadasUsuario/:id", async (req, res) => {
    ...
    axios.get('https://mi-repo-ten.vercel.app/usuarios/' + id)
        .then((respuesta) => {
            ...
            axios.get('https://mi-repo-ten.vercel.app/mapa/direccionCoordenadas/' + direccion)
                .then((respuesta) => {
...
router.get("/direccionUsuario/:id", async (req, res) => {
    ...
    try {
        const respuesta = await axios.get(`https://mi-repo-ten.vercel.app/usuarios/${id}`);
...
router.get('/coordenadasProducto/:id', async (req, res) => {
    try {
         const responseUsuario = await axios.get(`https://mi-repo-ten.vercel.app/usuarios/propietario/${req.params.id}`);
         const vendedorId = responseUsuario.data._id;
         const responseDireccionUsuario = await axios.get(`https://mi-repo-ten.vercel.app/mapa/direccionUsuario/${vendedorId}`);
         const { direccion } = responseDireccionUsuario.data;
         const responseCoordenadas = await axios.get(`https://mi-repo-ten.vercel.app/mapa/direccionCoordenadas/${direccion}`);
